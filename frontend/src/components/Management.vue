...
const fetchData = async (id: number) => {
  try {
    ...
    let response;
    if (id == 0){
      response = await fetch(`http://192.168.2.43:7779/management`,{
        headers: {
          'token' : store.getters.getToken,
        }
      });
    }else {
      response = await fetch(`http://192.168.2.43:7779/management/${id}`,{
        headers: {
          'token' : store.getters.getToken,
        }
      });
...
const onSubmitInitializeForm = async () => {
  ...
  try {
    const response = await fetch('http://192.168.2.43:7779/management/initializeItem',{
      method: 'POST',
      headers: {
...
const onSubmitInputDrawer = async () => {
  ...
  try {
    const response = await fetch('http://192.168.2.43:7779/management/inputItem', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'token' : store.getters.getToken,
...
const onSubmitOutputDrawer = async () => {
  ...
  try {
    const response = await fetch('http://192.168.2.43:7779/management/outputItem', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'token' : store.getters.getToken,
